...
describe('Reducers', () => {
  ...
  describe('Favorites Reducer', () => {
    ...
    it('should define the value of user favorites via POPULATE_FAVORITES', () => {
      ...
      const expected = [{ title: 'Arrival', poster: '/arrival.jpg' }]
      expect(favorites([], actions.populateFavorites(mockFavMovies))).toEqual(expected);
    })
    ...
    it('should add a favorited movie to the favorites key in store via ADD_FAVORITE', () => {
      ...
      const expected = [ { title: 'Dead Man', poster: 'deadMan.jpg'}, { title: 'The Thin Man', poster: 'thinMan.jpg'} ]
      expect(favorites(mockState, actions.addFavorite(mockFav))).toEqual(expected)
    })

    it('should remove an un-favorited movie from the favorites array in the store via REMOVE_FAVORITE', () => {
       const mockUnFav = { title: 'The Thin Man', poster: 'thinMan.jpg', id: 2};
      const mockState = [ { title: 'Dead Man', poster: 'deadMan.jpg', id: 1 }, { title: 'The Thin Man', poster: 'thinMan.jpg', id: 2} ]
      const expected = [ { title: 'Dead Man', poster: 'deadMan.jpg', id: 1} ]
      expect(favorites(mockState, actions.removeFavorite(mockUnFav))).toEqual(expected)
    })
  })
